...
class tree_updation extends Inf_Controller {
    ...
    function change_sponsor() {
        if($this->MODULE_STATUS['tree_updation'] != 'yes') {
            $note = '';
            $this->redirect($note, 'home/index');
        }
        ...
        if ($this->input->post('change_sponsor')) {
            ...
            if($this->tree_updation_model->isNewPlacementInsideTeam($user_id, $new_sponsor_id, 'sponsor_tree')) {
    ...
    function change_placement() {
        ...
        if ($this->input->post('change_placement')) {
            ...
            if($this->tree_updation_model->isNewPlacementInsideTeam($user_id, $new_placement_id, 'tree')) {
    ...
    public function refresh_tree_path()
    {
        $this->tree_updation_model->refreshTreepathTables();
    }
