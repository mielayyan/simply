...
class News extends Inf_Controller
{
    ...
    {   
        ...
        $this->form_validation->set_rules('news_title', lang('news_title'), 'trim|required|max_length[250]|strip_tags',[
           'required'=>lang('required'),
           'max_length'=>sprintf(lang('maxlength'),lang('news_title'),"50")
        ]);
        $this->form_validation->set_rules('news_desc', lang('news_desc'), 'trim|required',[
          "required"=>lang('required'),
          //'max_length'=>sprintf(lang('maxlength'),lang('news_desc'),"200")
        ]);
    ...
    {   
        ...
        $this->form_validation->set_rules('file_title', lang('File_Title'), 'trim|required|strip_tags|max_length[50]',[
          "required"=>lang('required'),
          'max_length'=>sprintf(lang('maxlength'),lang('File_Title'),"50")

        ]);
        $this->form_validation->set_rules('file_desc', lang('file_desc'), 'trim|required|strip_tags|max_length[200]',[
            "required"=>lang('required'),
          'max_length'=>sprintf(lang('maxlength'),lang('file_desc'),"200")
        ]);
        // NEXT_VERSION_ONLY
    ...
    public function add_new_news($action = '', $news_id = '') {
        ...
        $this->HEADER_LANG['page_small_header'] = '';
        if ($this->input->post('news_submit') && $this->validate_add_news()) {
            $news_image = "default.jpg";
            if (isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])) {
                $news_image = $this->uploadNewsImage('admin/add_new_news');
            }
            $news_title = (strip_tags($this->input->post('news_title', true)));
            //$news_desc = (strip_tags($this->input->post('news_desc', TRUE)));
            $news_desc = $this->input->post('news_desc', true);
            $result1 = $this->news_model->addNews($news_title, $news_desc, $news_image);
            if ($result1) {
                $data_array['news_title'] = $news_title;
                $data_array['news_description'] = $news_desc;
                $data_array['news_iamge'] = $news_image;
                $data = serialize($data_array);
                $this->validation_model->insertUserActivity($this->LOG_USER_ID, 'news added', $this->LOG_USER_ID, $data);
                // Employee Activity History
                if ($this->LOG_USER_TYPE == 'employee') {
        ...
        if ($action == "edit") {
            ...
            $this->set("news_title", $row['news_title']);
            $this->set("news_desc", $row['news_desc']);
            $this->set("news_image", $row['news_image']);
            $this->set("news_date", $row['news_date']);
        }
        if ($this->input->post('news_update') && $this->validate_add_news()) {
            $news_id1 = (strip_tags($this->input->post('news_id', true)));
            $news_title1 = (strip_tags($this->input->post('news_title', true)));
            $news_desc1 = $this->input->post('news_desc', true);
             $row = $this->news_model->editNews($news_id1);

            $news_image = $row['news_image'];
            if (isset($_FILES['image']['name']) && !empty($_FILES['image']['name']) ) {
                $news_image = $this->uploadNewsImage('admin/news/add_new_news/edit/'.$news_id1);
            }
            $result2 = $this->news_model->updateNews($news_id1, $news_title1, $news_desc1, $news_image);
            if ($result2) {
                $data_array['news_title'] = $news_title1;
                $data_array['news_description'] = $news_desc1;
                $data_array['news_id'] = $news_id1;
    ...
    public function uploadNewsImage($redirect_url = "admin/news/add_new_news") {
        $news_image = "";
        $random_number = floor($this->LOG_USER_ID * rand(1000, 9999));
        $config['file_name'] = "news_" . $random_number;
        $config['upload_path'] = IMG_DIR . 'news/';
        ...
        if ($this->upload->do_upload('image')) {
            ...
            $news_image = $data['upload_data']['file_name'];
            return $news_image;
        } else {
            ...
            switch($error['error']) {
                ...
                case "The upload path does not appear to be valid.":
                    $msg = lang('upload_path_not_valid');
                break;
                case "The uploaded file exceeds the maximum allowed size in your PHP configuration file.":
                    $msg = lang('upload_file_exceeds_limit_msg');
                break;
                case "The image you are attempting to upload doesn't fit into the allowed dimensions.":
                    $msg = lang("image_not_allowed_dimension");
                break;
                default:
        ...
        if ($this->input->post('new_faq') && $this->validate_add_faq()) {
            ...
            if ($question && $answer && $sort_order) {
                ...
                if ($ins_status) {
                    ...
                    if ($this->LOG_USER_TYPE == "employee") {
                        $details = $this->validation_model->insertEmployeeActivity($this->LOG_USER_ID, $this->LOG_USER_ID, $this->LOG_USER_TYPE, $activity);
                    } else {
                        $details = $this->validation_model->insertUserActivity("", $this->LOG_USER_ID, $this->LOG_USER_TYPE, $activity);
                    }
    ...
    {    
        $this->lang->load('validation');
        $this->form_validation->set_rules('sort_order', lang('sort_order'), 'trim|required|numeric|max_length[50]|greater_than[0]|callback_is_sortorder_available',[
          "required"=>lang('required'),
          'numeric'=>lang('digits'),
          'max_length'=>sprintf(lang('maxlength_digits'),lang('sort_order'),"50"),
          'greater_than'=>sprintf(lang('field_greater_than_zero'),lang('sort_order')),

        ]);
        $this->form_validation->set_rules('question', lang('question'), 'trim|required|max_length[200]',[
         "required"=>lang('required'),
         "max_length"=>sprintf(lang('maxlength'),lang('question'),"200")
        ]);
        $this->form_validation->set_rules('answer', lang('answer'), 'trim|required|max_length[1000]',[
        "required"=>lang('required'),
        "max_length"=>sprintf(lang('maxlength'),lang('answer'),"1000")
        ]);
