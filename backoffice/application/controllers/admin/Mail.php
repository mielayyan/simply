...
class Mail extends Inf_Controller {
    ...
    function read_mail($msg_id = "",$msg_type = "") {
        if(!$msg_id || !$msg_type) {
            $this->redirect("mail_management", 'mail/mail_management', FALSE);
        }
        ...
        // $msg_id = $this->encryption->decrypt($msg_id);

        // $msg_type = urldecode($msg_type);
        // $msg_type = str_replace("_", "/", $msg_type);
        // $msg_type = $this->encryption->decrypt($msg_type);
       ...
       // $thread = ($this->input->get('id', TRUE));
        ...
        // }
        $this->set('loged',$this->LOG_USER_ID);
    ...
    function validate_compose_mail() {
        ...
        $this->form_validation->set_rules('message1', lang('message'), 'trim|required|callback_validateMailContent',[
           "required"=>lang('required'),
           "validateMailContent"=>lang('required'),
        ]);
        $validate_form = $this->form_validation->run();
        return $validate_form;
    }

    public function validateMailContent($content)
    {
        return (strlen(trim(str_replace('&nbsp;', ' ', strip_tags($content)))) > 0);
    }

    function read_sent_mail($msg_id = "",$type = "") {
        if(!$msg_id || !$type) {
            $this->redirect("mail_management", 'mail/mail_sent', FALSE);
        }
        ...
        // $type = $this->encryption->decrypt($type);
