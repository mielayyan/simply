...
class Mail extends Inf_Controller
{
    ...
    function mail_management($tab = '')
    {
        ...
        for ($i = 0; $i < count($mail); $i++) {
            if($mail[$i]['type'] == 'contact') {
                ...
                // $mail_enc_thread = $this->encryption->encrypt($mail[$i]['mailtousid']);
                ...
                $id = $mail[$i]['thread'];
                // $mail_enc_thread = $this->encryption->encrypt($mail[$i]['thread']);
                // $mail_enc_thread = str_replace("/", "_", $mail_enc_thread);
                // $mail_enc_thread = rawurlencode($mail_enc_thread);
                // $mail[$i]['mail_enc_thread'] = $mail_enc_thread;
            ...
            // $mail_enc_type = $this->encryption->encrypt($mail[$i]['type']);
    ...
    {
        if(!$msg_id || !$msg_type) {
            $this->redirect("", 'mail/mail_management', false);
        }
        ...
        // $thread = $this->encryption->decrypt($thread);
    ...
    {
        if(!$msg_id || !$type) {
            $this->redirect("", 'mail/mail_sent', false);
        }
