...
class Repurchase extends Inf_Controller
{
    ...
    {
        parent::__construct();
        $this->load->library('internal_cart', ['user_id' => $this->LOG_USER_ID], 'cart');
        $this->load->model('configuration_model', '', true);
        $this->load->model('register_model', '', true);
        $this->load->model('product_model');
        if($this->uri->uri_string != "repurchase/check_ewallet_balance") {
    ...
    function repurchase_product()
    {
        ...
        $repurchase_details = $this->repurchase_model->getAllRepurchaseProducts();
            for ($i = 0; $i < count($repurchase_details); $i++) {
                $prod_enc_id = $repurchase_details[$i]['product_id'];
                //$this->encryption->encrypt($repurchase_details[$i]['product_id']);
                // $prod_enc_id = str_replace("/", "_", $prod_enc_id);
                // $prod_enc_id = str_replace("=", ":", $prod_enc_id);
    ...
    function checkout_product()
    {
        ...
        if ($this->MODULE_STATUS['product_status'] == 'yes') {
            ...
            $payment_module_status_array = $this->register_model->getPaymentModuleStatus();
              $payment_gateway_using_repurchse_status=$this->register_model->getPaymentGatewayUsingRegistration('repurchase');
        ...
        $this->set('payment_gateway_using_repurchse_status',$payment_gateway_using_repurchse_status);
    ...
    public function validate_checkout_address()
    {
        ...
        $this->form_validation->set_rules('full_name', lang('name'), 'trim|required|max_length[250]|callback__alpha_space',[
         "required"=>lang('required'),
         "max_length"=>sprintf(lang('maxlength'),lang('name'),"250"),
         "alpha_space"=>sprintf(lang('alpha_space'),lang('name'))
        ]);
        $this->form_validation->set_rules('address', lang('address'), 'trim|required|max_length[1000]',[
         "required"=>lang('required'),
         "max_length"=>sprintf(lang('maxlength'),lang('address'),"1000")

        ]);
        $this->form_validation->set_rules('pin_no', lang('pin_number'), 'trim|required|max_length[50]',[
             "required"=>lang('required'),
             "max_length"=>sprintf(lang('maxlength'),lang('pin_number'),"50")
        ]);
        $this->form_validation->set_rules('city', lang('city'), 'trim|required|max_length[250]|callback__alpha_city_address',[
           "required"=>lang('required'),
            "max_length"=>sprintf(lang('maxlength'),lang('city'),"250"),
             "alpha_city_address"=>lang('alpha_city_address'),
        ]);
        $this->form_validation->set_rules('phone', lang('phone_number'), 'trim|required|min_length[5]|max_length[50]|regex_match[/^[\s0-9+()-]+$/]',[
           "required"=>lang('required'),
           'min_length'=>sprintf(lang('minlength'),lang('phone_number'),"5"),
            "max_length"=>sprintf(lang('maxlength'),lang('phone_number'),"50")
    ...
    function repurchase_submit()
    {
        ...
        if (!empty($cart_products)) {
            if ($repurchase_post['active_tab'] == "epin_tab") {
                ...
                $ewallet_trans_password = $repurchase_post['tran_pass_ewallet'];
                if ($ewallet_user != "") {
                    if ($this->LOG_USER_TYPE == 'admin' || $this->LOG_USER_TYPE == 'employee') {
                        $admin_username = $this->validation_model->getAdminUsername();
                        if ($ewallet_user != $admin_username) {
                ...
                $pwallet_trans_password = $repurchase_post['tran_pass_pwallet'];
                if ($pwallet_user != "") {
                    if ($this->LOG_USER_TYPE == 'admin' || $this->LOG_USER_TYPE == 'employee') {
                        $admin_username = $this->validation_model->getAdminUsername();
    ...
    function product_details($product_enc_id)
    {
        ...
        $product_id = $product_enc_id;//$this->encryption->decrypt($product_id);
        $products = array();
...
//Purchase wallet ends
            ...
            if ($_FILES['file']['error'] != 4) {
                ...
                if (!$this->upload->do_upload('file')) {
                    ...
                    if ($this->session->userdata('reciept_name')) {
                        ...
                        $this->session->set_userdata('inf_payment_receipt',$doc_file_name);
                    } else {
    ...
    public function getRepurchaseInvoiceDetails(){

        $invoice_id = $this->input->post('invoice_id');

        $this->lang->load('report_lang');
        $invoice_details = array();
        $invoice_order_id = $this->validation_model->decrypt($invoice_id);
        ...
        $response = "<div class= 'button_back'>
    <a onClick='print_invoice_report(); return false;'><button class='btn m-b-xs btn-sm btn-primary btn-addon hidden-xs hidden-sm hidden-md'><i class='icon-printer'></i> " . lang('click_here_to_print') . "</button>
    </a>
</div>
<div class=' panel'>
<div class='panel-body' id='print_invoice_area'>
...
<div class='panel panel-default table-responsive ng-scope'>
    <table st-table='rowCollectionBasic' class='table table-bordered table-striped'>
       ...
       $counter = 1;
            ...
            foreach ($invoice_details['product_details'] as $v) {
             
             $response .= " <tr>
                    <td>" . $counter++ . " </td>
                    <td> {$v['product_name']} </td>
                    <td class='hidden-480'> {$v['quantity']} </td>
                    <td class='hidden-480'>" . format_currency($v['amount']) . "
                    </td>
       ...
       $response .= "<tr>
                  ...
                  <td colspan='2'><b>" .$this->DEFAULT_SYMBOL_LEFT ." 
                   " .number_format($total_amount * $this->DEFAULT_CURRENCY_VALUE,$this->PRECISION) . "
                 " . $this->DEFAULT_SYMBOL_RIGHT . "</b></td>
                </tr>
            </tbody>
    </table>
</div>
  ...
  exit;
    ...
    public function getPayoutInvoiceDetails(){

        $paid_id = $this->input->post('invoice_id');

        $this->lang->load('payout_lang');
...
<div class='panel-body' id='print_invoice_area'>
    ...
    <div class='row'>
        ...
        <div class='col-xs-6 text-right'>
            <p class='h4'>{$invoice_number}</p>" ;
             ...
             foreach ($payout_details as $v) {
               $response .=   "<h5>" . date("j F, Y",strtotime($v['paid_date'])) . "</h5>";
              } 
    ...
    <div class='col-sm-6'>
        ...
        <div class='well'><address>";
            ...
            $response .= "<strong>{$u['user_detail_name']}&nbsp;{$u['user_detail_second_name']}</strong>
...
<div class='panel panel-default table-responsive ng-scope'>
    ...
    public function orderDetailsInvoice(){
        ...
        $site_info = $this->validation_model->getSiteInformation();
        echo $this->load->view('admin/order/order_invoice', ['order_details' => $order_details] ,true);
        exit;
    }
    ...
    function test_rank(){
        ...
        if($new_rank>=4&&$old_rank<4) {
            ...
            print_r($gold_sponsor_upline);die;
            $user_ids = array();
    ...
    public function salesDetailsInvoice(){

        $invoice_no = $this->input->post('invoice_no');
        $user_id = $this->LOG_USER_ID;

        $order_details = $this->validation_model->getProductIdFromInvoice($invoice_no);

        $site_info = $this->validation_model->getSiteInformation();
        echo $this->load->view('user/report/order_invoice', ['order_details' => $order_details] ,true);
        exit;
    }
}
