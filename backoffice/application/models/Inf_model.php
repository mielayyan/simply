...
class inf_model extends Core_inf_model {
    ...
    public function getAllLanguages() {
        ...
        if ($multi_lang_status['lang_status'] == 'yes' && $multi_lang_status['lang_status_demo'] == 'yes') {
            ...
            $lang_arr[] = [
                'lang_id' => '1',
                'lang_code' => $this->APP_CONFIG['default_language']['code'],
                'lang_name' => 'English',
                'lang_name_in_english' => $this->APP_CONFIG['default_language']['name'],
                'flag_image' => '',
                'status' => 'yes',
                'default_id' => '1'
            ];
    ...
    public function getLeftMenu($user_id, $user_type, $current_url, $plan) {
        ...
        if($user_type == 'Unapproved'){
         ...
         $assigned_menus = "m#24,m#1";
        }
        if ($user_type == 'admin') {
            $permission_type = 'perm_admin';
        } else if ($user_type == 'agent'){
            $permission_type = 'perm_agent';
        ...
        if ($this->MODULE_STATUS['subscription_status'] == 'yes' && $this->MODULE_STATUS['product_status'] == 'yes' && $today > $package_validity_date && $user_type == 'user' && $product_id != 0) {
...
// dd($qry->result_array());
        foreach ($qry->result_array() as $row) {
            ...
            // Package Upgrade & Subscription Renewal in userside
            if ($row['menu_id'] == 71 OR $row['menu_id'] == 72) {
                continue;
            }
            ...
            // Report->Subscription
            if ($row['menu_id'] == 16 && $row['submenu_id'] == 222 && $this->MODULE_STATUS['subscription_status'] == "no") {
                continue;
            }
            
            // ProfileManagement->SubscriptionRenewal
            if ($row['menu_id'] == 19 && $row['submenu_id'] == 93 && $this->MODULE_STATUS['subscription_status'] == "no") {
                continue;
            }

            // Project Management -> Update PV
            if ($row['menu_id'] == 19 && $row['submenu_id'] == 157 && $this->MODULE_STATUS['product_status'] == "no") {
                continue;
            }

            // ProfileManagement->PackageUpgrade
            if ($row['menu_id'] == 19 && $row['submenu_id'] == 119 && $this->MODULE_STATUS['package_upgrade'] == "no") {
                continue;
            }

            // Report->PackageUpgrade
            if ($row['menu_id'] == 16 && $row['submenu_id'] == 220 && $this->MODULE_STATUS['package_upgrade'] == "no") {
                continue;
            }

            if($this->LOG_USER_TYPE == "user" && in_array($row['menu_id'], [71, 72])) {
                continue;
            }
    ...
    public function checkMenuPermitted($link_id, $perm_type = '', $menu_id = '', $assigned_menus = '') {
        ...
        $user_account_links = array(22, 11, 56, 63, 35, 69, 38, 157);
                ...
                if ($link_id != '#') {
                    ...
                    if (!$menu_id) {
                        ...
                        if ($menu_id == 0 && $sub_ref_id != 0 && $menu_ref_id) {
                            ...
                            if($menu_id == 32) { // Support Center == 32
                                $menu_check = 'm#' . $menu_id;
                            } else  {
    ...
    public function setDefaultLang($lang_code) {
        ...
        $query1 = $this->db->get('module_status');
        $multi_lang_status = $query1->row_array();
        if ($multi_lang_status['lang_status'] == 'no') {
            return;
        }
    ...
    public function getLanguageName($id) {
        ...
        if ($multi_lang_status['lang_status'] == 'yes' && $multi_lang_status['lang_status_demo'] == 'yes') {
            ...
            $query2 = $this->db->get('infinite_languages');
            return $query2->row_array()['lang_name_in_english'] ?? 'english';
        } else {
            return $this->APP_CONFIG['default_language']['name'];
        }
    }
    ...
    public function getDefaultLang($user) {
        ...
        return $query->row_array()['default_lang'] ?? 'english';
    }
    ...
    public function getPackageExpiredUserMenus() {
        ...
        $assigned_menus = "m#1,2#1,2#2,2#3,m#7,m#63,19#43,m#24,m#41,19#44,m#26,19#93";
    ...
    public function checkUrlPermitted($link_id, $perm_type) {
        ...
        if (count($query1->result_array()) > 0) {
            ...
            // Change to newui if sub_refid is 13 => pin_config
            if($this->LOG_USER_TYPE == "admin") {
                $sub_refid = $query1->row('sub_refid') == 13 ? 70 : $query1->row('sub_refid');
            } else {
                $sub_refid = $query1->row('sub_refid');
            }

            $menu_ok = $this->checkMenuStatus($sub_refid, $perm_type, $link_id);
    ...
    public function getDefaultLangUnapprovedUser($user) {
        $this->db->select('lang_status,lang_status_demo');
        $query1 = $this->db->get('module_status');
        $multi_lang_status = $query1->row_array();
        if ($multi_lang_status['lang_status'] == 'no') {
    ...
    public function getSystemStartDate()
    {
        ...
        $query = $this->db->get('ft_individual');
        return $query->row_array()['date_of_joining'];
    }
    ...
    public function getInfiniteMlmUserDetails($demo_id)
    {
        $dbprefix = $this->db->dbprefix;
        $this->db->set_dbprefix('');
        $array = $this->db->select('table_prefix,user_name,country')
        ->where('id', $demo_id)
            ->get("infinite_mlm_user_detail")->result_array();
        $this->db->set_dbprefix($dbprefix);
        return $array[0] ?? [];
    }
    ...
    public function getUserProfileImage($user_id) {
        ...
        $image = $query->row_array()['user_photo'] ?? 'nophoto.jpg';
        $image_path = IMG_DIR . 'profile_picture/' . $image;
        if (!file_exists($image_path)) {
            $image_path = IMG_DIR . 'profile_picture/nophoto.jpg';
        }
        
        return $image_path;
    }
    public function getAgentMenu($url){
            if($url=='epin/agent_epin'){
                $sel_home=0;
                $sel_epin=1;
                $sel_ewallet=1;
            }
            elseif($url=='ewallet/agent_wallet'){
                $sel_home=0;
                $sel_epin=0;
                $sel_ewallet=1;
            }else{
            ...
            $menu[0]['menu_id'] = 'agent_home';
            $menu[0]['submenu_id'] ='';
            $menu[0]['icon'] = 'fa fa-tachometer';
            $menu[0]['menu_status'] = 'yes';
            $menu[0]['submenu_status'] =''; 
            $menu[0]['menu_url'] = 'agent/home';
            $menu[0]['submenu_url'] ='';
            $menu[0]['menu_url_id'] = 7;
            $menu[0]['submenu_url_id'] ='';
            $menu[0]['menu_target'] = 'none';
            $menu[0]['submenu_target'] =''; 
            $menu[0]['menu_admin'] = 1;
            $menu[0]['menu_dist'] = 1;
            $menu[0]['menu_emp'] = 1;
            $menu[0]['submenu_admin'] ='';
            $menu[0]['submenu_dist'] ='' ;
            $menu[0]['submenu_emp'] =''; 
            $menu[0]['menu_order'] = 1;
            $menu[0]['submenu_order'] =''; 
            $menu[0]['is_menu_selected'] = $sel_home;
            $menu[0]['is_submenu_selected'] = 0;
            
            $menu[1]['menu_id'] = 'agent_epin';
            $menu[1]['submenu_id'] ='';
            $menu[1]['icon'] = 'fa fa-bookmark-o';//fa fa-tachometer
            $menu[1]['menu_status'] = 'yes';
            $menu[1]['submenu_status'] =''; 
            $menu[1]['menu_url'] = 'agent/epin';
            $menu[1]['submenu_url'] ='';
            $menu[1]['menu_url_id'] = 7;
            $menu[1]['submenu_url_id'] ='';
            $menu[1]['menu_target'] = 'none';
            $menu[1]['submenu_target'] =''; 
            $menu[1]['menu_admin'] = 1;
            $menu[1]['menu_dist'] = 1;
            $menu[1]['menu_emp'] = 1;
            $menu[1]['submenu_admin'] ='';
            $menu[1]['submenu_dist'] ='' ;
            $menu[1]['submenu_emp'] =''; 
            $menu[1]['menu_order'] = 1;
            $menu[1]['submenu_order'] =''; 
            $menu[1]['is_menu_selected'] = $sel_epin;
            $menu[1]['is_submenu_selected'] = 0;
                
            $menu[2]['menu_id'] = 'agent_ewallet';
            $menu[2]['submenu_id'] ='';
            $menu[2]['icon'] = 'fa fa-briefcase';
            $menu[2]['menu_status'] = 'yes';
            $menu[2]['submenu_status'] =''; 
            $menu[2]['menu_url'] = 'agent/ewallet';
            $menu[2]['submenu_url'] ='';
            $menu[2]['menu_url_id'] = 294;
            $menu[2]['submenu_url_id'] ='';
            $menu[2]['menu_target'] = 'none';
            $menu[2]['submenu_target'] =''; 
            $menu[2]['menu_admin'] = 1;
            $menu[2]['menu_dist'] = 1;
            $menu[2]['menu_emp'] = 1;
            $menu[2]['submenu_admin'] ='';
            $menu[2]['submenu_dist'] ='' ;
            $menu[2]['submenu_emp'] =''; 
            $menu[2]['menu_order'] = 2;
            $menu[2]['submenu_order'] =''; 
            $menu[2]['is_menu_selected'] = $sel_ewallet;
            $menu[2]['is_submenu_selected'] = 0;
            return $menu;
