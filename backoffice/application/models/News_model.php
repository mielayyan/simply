...
class news_model extends inf_model
{
    ...
    public function editNews($id)
    {
        ...
        foreach ($query->result_array() as $rows) {
            ...
            $obj_arr["news_image"] = $rows['news_image'];
    ...
    public function addNews($news_title, $news_desc, $news_image)
    {
        $date = date('Y-m-d H:i:s');
        $this->db->set('news_title', $news_title);
        $this->db->set('news_desc', $news_desc);
        $this->db->set('news_date', $date);
        $this->db->set('news_image', $news_image);
        $result = $this->db->insert('news');
        return $result;
    }

    public function updateNews($news_id, $news_title, $news_desc, $news_image)
    {
        $date = date('Y-m-d H:i:s');
        $this->db->set('news_title', $news_title);
        $this->db->set('news_desc', $news_desc);
        $this->db->set('news_date', $date);
        if($news_image != "" && $news_image != "default.png") {
            $this->db->set('news_image', $news_image);
        }
    ...
    public function getAllNews($limit="", $offset="")
    {
        ...
        if ($limit>0) {
            $this->db->order_by("news_date", "desc","news_id");
    ...
    public function getAllLatestNews($id="")
    {
        $this->db->order_by("news_date", "desc","news_id");
        if ($id) {
          $this->db->where("news_id",$id);
