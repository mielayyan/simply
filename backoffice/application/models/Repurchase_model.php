...
class Repurchase_model extends Inf_model {
    ...
    public function getEpinDetails($epin, $user_id, $upgrade_user_id='')
    {
        ...
        $this->db->limit(1);
        $res = $this->db->get('pin_numbers')->row_array();
        if ($res) {
            return $res;
        }
        else {
    ...
    public function getPendingOrders($order_id= '',$user_id = "", $page= '', $limit= '') {

        $details = [];
        $this->db->select('r.*, f.user_name');
        $this->db->from('repurchase_order as r');
        $this->db->join('ft_individual as f', 'f.id=r.user_id');
