...
class currency_model extends inf_model {
    ...
    public function updateUserCurrency($id, $user_id) {
        $code = $this->db->select("code")->where("id", $id)->get("currency_details")->result_array()[0]["code"] ?? "";
        if(!$code)
            return false;
        ...
        if($this->MODULE_STATUS["opencart_status"] == "yes") {
            $oc_currency_id = $this->db->select("currency_id")->where("code", $code)->get("oc_currency")->result_array()[0]['currency_id'] ?? "";
            if($oc_currency_id) {
                $this->setOcCurrencySession($code);
            }
        }
    ...
    public function getMultyCurrencyStatus() {
        ...
        $multy_currency_status = $query->row_array()['multy_currency_status'];
        return ($multy_currency_status == 'yes');
    }
    ...
    public function getProjectDefaultCurrencyDetails() {
        ...
        if ($currency_status == 'no') {
            $currency_details = array(
                'id' => '1',
                'title' => 'dollar',
                'code' => $this->APP_CONFIG['default_currency']['code'],
                'value' => $this->APP_CONFIG['default_currency']['value'],
                'symbol_left' => $this->APP_CONFIG['default_currency']['symbol_left'],
                'symbol_right' => $this->APP_CONFIG['default_currency']['symbol_right'],
                'decimal' => '',
                'status' => 'enabled',
                'default_id' => '1',
                'delete_status' => 'yes',
    ...
    public function changeCurrencyStatus($ids = [], $status = "enabled") {
        $this->db->set('status', $status);
        $this->db->where_in('id', $ids);
        $this->db->update('currency_details');
        return $this->db->affected_rows() >= 1 ? true : false;
    }
    public function updateUserCurrencyForUnapproved($id,$user_id){
    ...
    public function getUserDefaultCurrencyDetailsForUnapproved($user_id = '') {
        $currency = NULL;
        $this->db->select('default_currency');
    ...
    public function setOcCurrencySession($currency_code)
    {
        $this->load->model("multi_language_model");
        $oc_sess_data = $this->multi_language_model->get_store_session_data();
        
        if (isset($_COOKIE['OCSESSID'])) {
            
            if(isset($oc_sess_data['customer_id'])) {
                $oc_sess_data['currency'] = $currency_code;
            }
