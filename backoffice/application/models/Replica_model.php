...
//Edited From here
   ...
   // validation
    ...
    function GetReplicaContent($lang_id,$user_id=NULL) {
        ...
        $query =$this->db->get();
        $replica_content = $query->result();
        $data = [];
        foreach ($replica_content as $key => $value) {
            $data[$value->key] = xss_clean($value->value);
        }
        return $data;
    }
    ...
    {
        $user_details = $this->db->select("CONCAT(user_detail_name,' ', user_detail_second_name) as fullname, user_detail_email as email, user_detail_mobile as phone")
                ->where('user_detail_refid', $user_id)
                ->get('user_details')->row_array();
        return $user_details;
    }
