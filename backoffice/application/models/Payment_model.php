...
class payment_model extends inf_model {
    ...
    {
        $where = ['gateway_name' => 'Paypal', 'status' => 'yes'];
        return ($this->db->where($where)->count_all_results('payment_gateway_config') > 0);
    }
    ...
    public function addStripePaymentHistory($data) {
    ...
    public function updateStripePaymentHistory($co_session_id, $params)
    {
        return $this->db->where('session_id', $co_session_id)->update('stripe_payment_history', $params);
    }
    public function getStripePaymentHistory($session_id, $arr_select)
    {
        return $this->db->select(implode(',', $arr_select))->from('stripe_payment_history')->where('session_id', $session_id)->get()->row();
    }
