...
class Inf_smarty extends Smarty {
	...
	function view($template, $data = array(), $return = FALSE)
	{
		...
		$replica_pages = ['replica_preview','replica_blockchain','replica_bitgo_gateway','replica_authorizeNetPayment','replica_pay_now','replica_payeer','replica_sofort_payment','replica_sofort_register','replica_squareup_gateway'];
		...
		if ($CI->LOG_USER_ID && (uri_string() != 'replica_register' && !in_array($CI->uri->segment(1),$replica_pages))) {
			...
			} elseif (in_array($CI->LOG_USER_TYPE, ['user','Unapproved'])) {
		...
		if ($return == FALSE)
		{
			...
			$script_src = "script-src *  'unsafe-inline' 'unsafe-eval' https://cdnjs.cloudflare.com https://www.statcounter.com https://embed.tawk.to https://ajax.googleapis.com https://code.jquery.com; ";
	...
	public function setFormErrorFromFlashData($CI)
	{
		if ($CI->session->flashdata('form_error_redirect')) {
            ...
            $CI->form_validation->set_error_delimiters("<span style='color:#b94a48;' class='help-block-2'>", "</span>");
        }
        if ($CI->session->flashdata('form_input_values')) {
            foreach ($CI->session->flashdata('form_input_values') as $field => $value) {
            	$CI->form_validation->set_form_data($field, $value);
            }
        }
	}
