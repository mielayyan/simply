...
trait REST_Controller {
     ...
     */
    public function __construct($config = 'rest_api')
    {
        // parent::__construct();
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        {
            $this->set_error_response(REST_Controller_Definitions::HTTP_FORBIDDEN, 1020);
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->set_error_response(REST_Controller_Definitions::HTTP_FORBIDDEN, 1002);
        }
        ...
        if (! method_exists($this, $controller_method))
        {
            $this->set_error_response(REST_Controller_Definitions::HTTP_METHOD_NOT_ALLOWED);
        }
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $this->set_error_response(REST_Controller_Definitions::HTTP_TOO_MANY_REQUESTS);
            }
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            ...
            if ($this->config->item('rest_key_expiry') === TRUE && $this->config->item('rest_key_expiry_time') > 0) {
                ...
                if (time() >= $row->date_created + $this->config->item('rest_key_expiry_time')) {
                    $this->rest->db->where($this->config->item('rest_key_column'), $key)->delete($this->config->item('rest_keys_table'));
                    return FALSE;
                }
            }
