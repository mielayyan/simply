...
{if $DEFAULT_SYMBOL_LEFT && $obj_arr['level_commission_type'] == "flat"}
    {$input_group_hide = ""}
    {$left_symbol = "<span class='input-group-addon'>$DEFAULT_SYMBOL_LEFT</span>"}
{/if}
{if $DEFAULT_SYMBOL_RIGHT && $obj_arr['level_commission_type'] == "flat"}
    {$input_group_hide = ""}
            ...
            {if $level < $obj_arr['depth_ceiling']}
                ...
                <div class="form-group">
                    ...
                    <div class="input-group {$input_group_hide}">
                        {$left_symbol}
                        <input type="text" class="level_percentage form-control" name="level_percentage{$level}" data-lang="{lang('you_must_enter')} {if $MODULE_STATUS['xup_status'] == "yes"} {lang('X-UP')} {/if} {lang('level')} {$level} {lang('commission_')}" id="level_per{$level}" min="0" {if $obj_arr["level_commission_type"] != "percentage"} value="{number_format($levl_perc*$DEFAULT_CURRENCY_VALUE,2, '.', '')}"{else} value="{$levl_perc}"{/if}>
                        {$right_symbol}
                    </div>
                        ...
                        <tr>
                            ...
                            {foreach from=$arr_pck item=v}
                                <td>
                                    <div class="input-group {$input_group_hide}">
                                        {$left_symbol}
                                        <input type="text"  class="level_percentage form-control" name="level_{$u['level']}_{$v['prod_id']}_reg" data-lang="{lang('you_must_enter')} {lang('level')} {$u['level']} {$v['prod_id']} {lang('commission_')}" id="level_{$u['level']}_{$v['prod_id']}_reg" min="0" 
                                        {if $obj_arr["level_commission_type"] != "percentage"}
                                            {if isset($u[{"{$v['prod_id']}reg"}])}
                                                value="{number_format($u[{"{$v['prod_id']}reg"}]*$DEFAULT_CURRENCY_VALUE,2, '.', '')}"
                                             {/if} 
                                        {else} 
                                            {if isset($u[{"{$v['prod_id']}reg"}])}
                                                value="{$u[{"{$v['prod_id']}reg"}]}"
                                            {/if}
                                        {/if}>
                                        {$right_symbol}
                                    </div>
                        ...
                        <tr>
                            ...
                            {foreach from=$arr_pck item=v}
                                <td>
                                    <div class="input-group {$input_group_hide}">
                                        {$left_symbol}
                                        <input type="text"  class="level_percentage form-control" name="level_{$u['level']}_{$v['prod_id']}_member" data-lang="{lang('you_must_enter')} {lang('level')} {$u['level']} {$v['prod_id']} {lang('commission_')}" id="level_{$u['level']}_{$v['prod_id']}_member" min="0" 
                                            {if $obj_arr["level_commission_type"] != "percentage"} 
                                                value="{number_format($u[{"{$v['prod_id']}member"}]*$DEFAULT_CURRENCY_VALUE,2, '.', '')}"
                                            {else} 
                                                value="{$u[{"{$v['prod_id']}member"}]}"
                                            {/if}>
                                        {$right_symbol}
                                    </div>
