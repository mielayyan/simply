...
{block name=$CONTENT_BLOCK}
    ...
    {include file="layout/search_member_post.tpl"}
        ...
        <div class="col-sm-6">
            <div class="panel panel-default">
                ...
                <div class="list-group no-radius alt">
                    ...
                    <div class="list-group-item" href=""> <span class="badge bg-info">{format_currency($current_package_details['price'])}</span>{lang('package_price')}</div>
                </div>
            </div>
        </div>
                    ...
                    <div class="form-group">
                        <select name="product_id" id="product_id" class="form-control m-b">
                            ...
                            {foreach from = $upgradable_package_list item = v}
                                <option value="{$v.product_id}">{$v.product_name} ({format_currency($v.price)})</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
