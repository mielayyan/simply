...
<div id="span_js_messages" style="display: none;">
 ...
 <div class="m-b pink-gradient">
    ...
    <div class="panel panel-default">
        {form_open('admin/member/approve_pending_subscription', 'name="pending_reg" class="" id="pending_reg" method="post"')}
    <input type="hidden" id="details" value='{json_encode($pending_subscription_list)}'>
    <div class="table-responsive">
        <table class="table table-striped m-b-none">
            ...
            <tbody>
                {if count($pending_subscription_list)}
                {assign var="i" value=1}
                {foreach from = $pending_subscription_list key = k item = v}

                    <tr>
                        <td>
                            <div class="checkbox">
                                <label class="i-checks i-checks-sm">
                                    <input type="checkbox" name="release[]" id="release{$i}" class="release" value="{$v['pending_id']}"/><i> </i>
                                </label>
                            </div>
                        </td>
                        <td>{$v['user_name']}</td>
