...
{block name=$CONTENT_BLOCK}
    ...
    <p class="">
        <a href="{BASE_URL}/admin/unilevel_history?from_tree=1" class="btn btn-sm btn-info btn-addon"><i class="fa fa-forward"></i>{lang('go_to_unilevel_list')}</a>
        {if $MODULE_STATUS['tree_updation'] == 'yes'}
        <a href="{BASE_URL}/admin/change_sponsor" class="btn btn-sm btn-info btn-addon pull-right"><i class="fa fa-users"></i>{lang('change_sponsor')}</a>
        {/if}
    </p>

    {include file="layout/search_tree.tpl" search_url="admin/sponsor_tree"}
    
    <div id="summary" class="tree_main" style="overflow: hidden; position: relative;">
        {include file="admin/tree/tree_view_sponsor.tpl" search_url="admin/sponsor_tree"}
    </div>
