...
{block name=$CONTENT_BLOCK}
    ...
    <p class="text-right">
        <a href="{BASE_URL}/admin/binary_history?from_tree=1" class="btn btn-sm btn-info btn-addon"><i class="fa fa-forward"></i>{lang('downline_list')}</a>
        {if $MODULE_STATUS['tree_updation'] == 'yes'}
        <a href="{BASE_URL}/admin/change_placement" class="btn btn-sm btn-info btn-addon pull-right"><i class="fa fa-users"></i>{lang('change_placement')}</a>
        {/if}
    </p>
    
    {include file="layout/search_tree.tpl" search_url="admin/genology_tree"}
    
    <div class="panzoom-parent">
        <div id="summary" class="tree_main" style="overflow: hidden; position: relative;">
            {include file="admin/tree/tree_view.tpl"}
        </div>
    </div>
...
{block name=script}
    {$smarty.block.parent}
    <script src="{$PUBLIC_URL}javascript/panzoom/panzoom.js"></script>
