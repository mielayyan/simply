	...
	<div class="hbox hbox-auto-xs hbox-auto-sm" ng-controller="MailCtrl">
	      ...
	      <ul class="list-group list-group-lg no-radius   m-t-n-xxs">
                ...
                {foreach from=$adminmsgs item=v}
                    ...
                    {$msg_id=$v.id}
                    {if $v.type == 'contact'}
                    {$url = "`$BASE_URL`admin/mail/read_sent_mail/`$v['id']`/`$v['type']`"}
                    {else}
                    {$url = "`$BASE_URL`admin/mail/read_sent_mail/`$v['thread']`/`$v['type']`"}
                    {/if}
                        <li class="list-group-item clearfix b-l-3x b-l-info">
                            ...
                            <a href="{$url}">
                            <div class="clear">
                                <div><span  class="text-md ">{if $v.type == 'team'}ALL{else} {$v.fullname}({$user_name}){/if}</span></div>
