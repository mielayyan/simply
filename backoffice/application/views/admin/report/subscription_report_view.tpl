...
{assign var="excel_url" value="{$BASE_URL}admin/excel/create_excel_subscription_report?username={$userName}"}
{assign var="csv_url" value="{$BASE_URL}admin/excel/create_excel_subscription_report?username={$userName}"}

<div class="panel panel-default">
  <div class="panel-body">
  {form_open('admin/subscription_report','role="form" class="" method="get"  name="searchform" id="searchform"')}
            ...
            <tbody>
                <thead>
                    <tr class="th">
                        ...
                        {if $showPackage == "yes"}<th>{lang('package')}</th>{/if}
                        <th>{lang('subscription_amount')}</th>
                        <th>{lang('payment_method')}</th>
                        <th>{lang('subscription_date')}</th>
                    </tr>
                </thead>
                {assign var="i" value=0}{assign var="total" value=0} {foreach from=$details item=v} {$i=$i+1}
                <tr>
                    <td>{$ci->input->get('offset')+$i}</td>
                    {if $filterUserString == ""}<td>{$v.username}({$v.user_full_name})</td>{/if}
                    {if $showPackage == "yes"}<td>{$v.package_name}({format_currency($v.package_amount)})</td>{/if}
                    <td>{format_currency($v.total_amount)}</td>
                    <td>{lang($v.payment_type_used)}</td>
                    <td>{date("d M Y - h:i:s A", strtotime($v.date_submitted))}</td>
                </tr>
                ...
                <tr>
                    ...
                    {if $showPackage != "yes"}{$colspan = $colspan - 1}{/if}
