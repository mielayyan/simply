...
<style>
    ...
    .bg-white-only #logo_edit_icon {
        color: black;
    }
    .app-aside-folded #logo_edit_icon {
        display: none;
    }
</style>
...
<header id="header" class="app-header navbar" role="menu">
    ...
    <div class="navbar-header bg-dark">
        ...
        <a {if $LOG_USER_TYPE=='agent'}href="{$BASE_URL}agent/home" {else}href="{$BASE_URL}admin/home/index"{/if} class="navbar-brand text-lt">
            {* <i class="fa fa-btc"></i> *}
            <img src="{$SITE_URL}/uploads/images/logos/{$site_info["logo"]}" alt="." class="">
            <img src="{$SITE_URL}/uploads/images/logos/{$site_info["logo_shrink"]}" alt="logo" class="logo-shrink">
            {* <span class="hidden-folded m-l-xs">Angulr</span> *}
        </a>
        {if $LOG_USER_TYPE != "user"}
            <a href="{$BASE_URL}admin/site_information" title="{lang('edit_logo')}" id="logo_edit_icon" class="hidden-sm hidden-xs">
    ...
    <div class="collapse pos-rlt navbar-collapse box-shadow bg-white-only">
        ...
        <ul class="nav navbar-nav navbar-right">
            {if $MODULE_STATUS['opencart_status_demo']=="yes" && $MODULE_STATUS['opencart_status']=="yes"}
             ...
             {if isset($latest_orders) && !empty($latest_orders)}
            <li class="dropdown">
                <a href="#" data-toggle="dropdown" class="dropdown-toggle">
            ...
            <li class="dropdown">
                ...
                <div class="dropdown-menu w-xl animated fadeInUp">
                    <div class="panel bg-white">
                        ...
                        <div class="list-group">
                            {if $payout_count>0}
                            <a href="{$BASE_URL}admin/payout?tab=requests" class="list-group-item">
                                <i class="fa fa-fw fa-bullhorn"></i>
                                {lang('you_have_n_payout')|sprintf:$payout_count}
                            </a>
                            {/if}
                            {if $pin_count>0 && $MODULE_STATUS['pin_status']=="yes"}
                            <a href="{$BASE_URL}admin/epin?tab=requests" class="list-group-item">
            ...
            <li class="dropdown">
                ...
                <ul class="dropdown-menu animated fadeInRight w">
                    {if $LOG_USER_TYPE != "agent"}
                     {if $LOG_USER_TYPE == "user"}
                       <li> <a href="{$PATH_TO_ROOT}{$LOG_USER_TYPE}/profile/profile_view">
                            <span>{lang('profile_management')}</span>
                        </a></li>
                    {else}
                        <li><a href="{$PATH_TO_ROOT}admin/profile/profile_view">
                            <span>{lang('profile_management')}</span>
                        </a></li>
                    {/if}
                    <li>
                    ...
                    {else}
                        <li> <a href="{$PATH_TO_ROOT}{$LOG_USER_TYPE}/member/agent_profile_view">
                            <span>{lang('profile_management')}</span>
                        </a></li>
                    {/if}
