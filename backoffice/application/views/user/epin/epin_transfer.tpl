   ...
   <table st-table="rowCollectionBasic" class="table table-striped">
      ...
      <tbody>
         ...
         <tr class="{$class}">
            ...
            <td>{format_currency($v.pin_amount)}</td>
            <td>{format_currency($v.pin_balance_amount)}</td>
            {*
            <td>{$used_user}</td>
            *}
            ...
            <td>
                    ...
                    <span onclick="return copyEpinToClipboard(link{$i}, 'link{$i}')" class="btn-link btn_size has-tooltip text-primary" style="cursor: pointer;" title="{lang('copy_clipboard')}"><i class="fa fa-clipboard" ></i></span>
                ...
                {if $v.status == "yes" && $v.purchase_status =="yes" && ($v.pin_balance_amount > 0)}
                    <!--refund option-->
                        ...
                        {else}
                            <a class='btn-link btn_size has-tooltip text-danger' onclick="javascript:refund_pin({$v.pin_id}, '{$root}')" title="{lang('refund')}"><i class="icon-reload"></i></a>
                        {/if}
                    <!--refund option-->
                {/if}
            </td>
