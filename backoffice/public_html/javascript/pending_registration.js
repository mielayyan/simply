...
// function to initiate Validation Sample 1
	$('#user_detail_modal').on('show.bs.modal', function (e) {
        var key = $(e.relatedTarget).data('key');
        setText('.user_name', details[key].user_name);
        setText('.modal-sponsor', `${details[key].sponsor_full_name} (${details[key].sponsor_user_name})`)
        setText('.sponsor_full_name', details[key].sponsor_full_name);
        setText('.package', `${details[key].package_name} (${trans('amount')}: ${format_currency(details[key].product_amount)})`);
        ...
        setText('.address_line1', details[key].adress_line1);
        setText('.address_line2', details[key].adress_line2);
        setText('.country', details[key].country);
        ...
        // If user is registered from tree show placement username.
        if(details[key].reg_from_tree == "1") {
            $('#user_detail_modal .user-info-tbody .reg_from_tree').removeClass('hidden');
            setText('#user_detail_modal .user-info-tbody .reg_from_tree .placement_user', `${details[key].placement_full_name} (${details[key].placement_user_name})`);
        } else {
            $('#user_detail_modal .user-info-tbody .reg_from_tree').addClass('hidden');
            setText('#user_detail_modal .user-info-tbody .reg_from_tree .placement_user', '');
        }
    });
    ...
    $('#confirm_registr').click(function() {
        $('#pending_reg').submit();
    });
