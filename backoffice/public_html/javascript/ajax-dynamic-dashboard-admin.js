...
$(function() {
    setTimeout(function () {
        $('.demo-note').addClass('demo-note-display');
    }, 1500);
    setTimeout(function () {
        $('.demo-note').removeClass('demo-note-display');
    }, 15000);
    $('.demo-note > button.close').on('click', function () {
        $(this).parent('.demo-note').removeClass('demo-note-display');
    });
    ...
    // Doughnut
    var doughnutData = {
        labels: doughnutLabelsArray,
        datasets: [{
            data: doughnutDataArray,
            backgroundColor: ["#8064a2", "#4bacc6", "#f79646"]
        }]
    };
        ...
        tooltips: {
            callbacks: {
                label: function(tooltipItem, data) {
                    let sum = doughnutDataArray.reduce((a, b) => a + b, 0);
                    let percentage = parseFloat((doughnutDataArray[tooltipItem.index]/sum)*100).toFixed(2);
                    return data.labels[tooltipItem.index]+": "+doughnutDataViewArray[tooltipItem.index]+" ("+percentage+" %)";
                }
            }
        }
    };
