...
function validate_ewallet() {
    ...
    disable_ewallet();
    $('#user_name_ewallet_box').hide();

    if(ewallet_username == "" || ewallet_password == "") {
        if(ewallet_username == "") {
            $("#user_name_ewallet_box").fadeTo("fast", 1, function() //start fading the messagebox
            {
                //var msg37 = $("#validate_msg61").html();
                var msg41=trans('required',trans('username'));
                $(this).removeClass();
                $(this).addClass('messageboxerror');
                $(this).html('<i style="color: red;" class="fa fa-times-circle"></i>' + msg41).show().fadeTo("fast", 1);
                disable_ewallet();
            });
        } else if(ewallet_username != "") {
             $.ajax({
                'url': base_url + 'admin/member/get_username_availability',
                'type': "POST",
                'data': {
                    username: $('#user_name_ewallet').val(),
                },
                success: function(data) {
                    
                    $usernameusing=$('#user_name_ewallet').val();
                    if($usernameusing!="")
                    {
                    if(data=='no')
                    {
                       $("#user_name_ewallet_box").fadeTo("fast", 1, function() {
                            var msg43 = trans('invalid',trans('username'));
                            $(this).removeClass();
                            $(this).addClass('messageboxerror');
                            $(this).html('<i style="color: red;" class="fa fa-times-circle"></i>' + msg43).show().fadeTo("fast", 1);
                            disable_ewallet();
                        }); 
                    }
                    if(data=="yes"){
                        $("#user_name_ewallet_box").fadeTo("fast", 1, function() {
                            var msg45 = trans('valid',trans('username'));
                            $(this).removeClass();
                            $(this).addClass('messageboxok');
                            $(this).html('<img align="absmiddle" src="' + path_temp + 'images/accepted.png" />' + msg45).show().fadeTo("fast", 1);
                            enable_ewallet();
                        }); 
        ...
        if(ewallet_password == "") {
            ...
            {
                //var msg37 = $("#validate_msg61").html();
                var msg37=trans('required',trans('trans_passwd'));
                $(this).removeClass();
                $(this).addClass('messageboxerror');
       ...
       var msg100= trans('invalid',trans('trans_passwd'));
        $("#tran_pass_ewallet_box").removeClass();
        $("#tran_pass_ewallet_box").addClass('messagebox');
        $("#tran_pass_ewallet_box").html('<img align="absmiddle" src="' + path_temp + 'images/loader.gif" /> ' + msg100).show().fadeTo("fast", 1);
        disable_ewallet();
         });
        $.post(ewallet_available, {
            ...
            if (data == "yes") {
                $("#tran_pass_ewallet_box").fadeTo("fast", 1, function() {
                    ...
                    var msg39=trans('valid',trans('trans_passwd'));
                    $(this).removeClass();
                    $(this).addClass('messageboxok');
                ...
                $("#tran_pass_ewallet_box").fadeTo("fast", 1, function() {
                    //var msg37 = $("#validate_msg61").html();
                    var msg37=trans('invalid',trans('trans_passwd'));
                    $(this).removeClass();
                    $(this).addClass('messageboxerror');
                    $(this).html('<i style="color: red;" class="fa fa-times-circle"></i>' + msg37).show().fadeTo("fast", 1);
                    disable_ewallet();
                ...
                $("#tran_pass_ewallet_box").fadeTo("fast", 1, function() {
                    //var msg37 = $("#validate_msg61").html();
                    var msg37=trans('insufficient_balance');
                    $(this).removeClass();
                    $(this).addClass('messageboxerror');
                    $(this).html('<i style="color: red;" class="fa fa-times-circle"></i>' + msg37).show().fadeTo("fast", 1);
                    disable_ewallet();
