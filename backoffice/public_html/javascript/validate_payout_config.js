    ...
    var runPayoutValidation = function () { 
        ...
        $('#payout_form').validate({
            ...
            messages: {
                min_payout: {
                    number: trans('digits'),
                    minStrict: trans('equal_zero'),
                    required:trans('minimum_payout'),
                    maxlength:trans('maxlength_digits',trans('min_payout'),"10")
                },
                max_payout: {
                    number: trans('digits'),
                    minStrict: trans('equal_zero'),
                    required:trans('maximum_payout'),
                    maxlength: trans('maxlength_digits',trans('max_payout'),"10")
                },
                payout_validity: {
                    digits:trans('digits'),
                    maxlength: trans('digit_limit'),
                    required: trans('payout_validity1')
                },
                payout_fee_amount: {
                    number: trans('digits'),
                    min: trans('equal_zero'),
                    maxlength: trans('digit_limit'),
                    required: trans('payout_fee_amount_required')
                }
            },
...
$("#payout_fee_mode").change(function () {
    var mode = $(this).val();
    if(mode === 'percentage') {
        $("#payout_fee_amount").parent('.input-group').addClass("input-group-hide");
        $("#payout_fee_amount").rules('add', {
            "max" : 100,
            "messages" : {
                "max" : trans('not_exceed_100'),
            }
        });
    }
    if(mode === 'flat') {
        $("#payout_fee_amount").parent('.input-group').removeClass("input-group-hide");
        $("#payout_fee_amount").rules("remove", "max");
    }
})