...
function updateSignupSettings(key, value, element) {
    return;
    var base_url = $('#base_url').val();
    var switch_elements = ['registration_allowed', 'referral_status', 'mail_notification','compression_commission']
    $.ajax({
...
$('#signup_settings_form').validate({
    ...
    messages: {
        length: {
          required: trans('required',trans('user_name_length')),  
          digits: trans('digits'),
          maxlength:trans('maxlength',trans('user_name_length'),"2")
        }, 
        prefix: {
            required: msg12, 
        }
...
$('.panel-default').find('.next_button').on("click", function() {
        var element = $(this).closest('.panel-default');
    $('.panel-default').closest('.panel-default').show();
        element.show();
        $('html, body').animate({
    ...
    function delete_custom(id) {
        ...
        if (confirm(confirm_msg)) {
            document.location.href = $('#base_url').val() + 'admin/configuration/delete_custom_signup/' + id;
        }
    }

    function edit_custom(id, root) {
        document.location.href = root + 'configuration/edit_custom_settings/' + id;
    }

    var ValidateCustomField = function () {
        var runValidatorUpdation = function () {
    
            var msg7 = $("#validate_msg1").html();
            var msg8 = $("#validate_msg2").html();
            var msg9= $("#digit_only").html();
            var searchform = $('#custom_field_form');
            var errorHandler1 = $('.errorHandler', searchform);
