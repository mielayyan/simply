...
var ValidatePayoutRelease = function() {
    ...
    var payoutReleaseRequest = function() {
        ...
        $.validator.addMethod('greater_than', function(value) {
            return value > 0;
        }, "<font color='#b94a48'>" + msg6 + "</font>");
        $('#payout_request').validate({
            ...
            messages: {
                payout_amount: {
                    required: trans('withdraw_amount_enter'),
                    number: trans('digits'),
                    greater_than:trans('field_greater_than_zero',trans('amount'))
                },
                transation_password: {
                    required: trans('transation_password_enter'),
                    minlength: trans('minlength',trans('6')),
                    maxlength: trans('maxlength',trans('100'))
                }
...
$('.release_requests_all').click(function() {
    $(this).is(':checked') ? $('.payout-checkbox').prop('checked', true) : $('.payout-checkbox').prop('checked', false);
...
$("#payout_amount").change(function () {
    var payoutAmount= $(this).val() * 1;
    var feeAmount = $("#payout_fee_amount").val();
    var feeMode = $("#payout_fee_mode").val();
    if(payoutAmount && $(this).valid()) {
        var deduction = (feeAmount * $('#DEFAULT_CURRENCY_VALUE').val()).toFixed($('#DEFAULT_PRECISION').val());
        if (feeMode === "percentage") {
            deduction = (payoutAmount * feeAmount / 100).toFixed($('#DEFAULT_PRECISION').val());
        }
        if(parseFloat(deduction) > parseFloat(0)) {
            var dedText = trans('payout_fee_ded_text', $('#DEFAULT_SYMBOL_LEFT').val() + deduction + $('#DEFAULT_SYMBOL_RIGHT').val());
            $("#payout_amount_text").empty();
            $("#payout_amount_text").append(dedText);
            $("#payout_amount_text").parent().removeClass("no-display");
        } else {
            $("#payout_amount_text").empty();
            $("#payout_amount_text").parent().addClass("no-display");
        }
    } else {
        $("#payout_amount_text").empty();
        $("#payout_amount_text").parent().addClass("no-display");
