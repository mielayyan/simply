$(function () {
	...
	var valid_span = '<span id="msg_epin_valid"><i style="color: green;" class="fa fa-check"></i>&nbsp;Valid E-Pin</span>';
        ...
        var duplicate_span = '<span id="msg_epin_invalid"><i style="color: red;" class="fa fa-times"></i>&nbsp;please enter E-Pin</span>';
        var repeted_span = '<span id="msg_epin_invalid"><i style="color: red;" class="fa fa-times"></i>&nbsp;Repeated E Pin</span>';
	...
	$('#check_epin').on('click', function () {
		...
		$('table#epin_table > tbody > tr > td:nth-child(2) > input').each(function (i) {
             ...
             var epin_name=$("input[name='epin[]']").val();
           ...
           if(epin_name=="")
           {
			...
			if ((!epin || epin_exists) && epin != '') {
                ...
                status = false;
                $('table#epin_table > tbody > tr:eq(' + i + ')').find('td:nth-child(2) > span').remove();
				$('table#epin_table > tbody > tr:eq(' + i + ')').find('td:nth-child(2) > input').after(repeted_span);
          //                      $('table#epin_table > tbody > tr:eq(' + i + ')').find('td:nth-child(2) > span').remove();
			       // $('table#epin_table > tbody > tr:eq(' + i + ')').find('td:nth-child(2) > input').after(duplicate_span)
			}
			else {
