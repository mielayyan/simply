...
$(function () {
    ...
    $("#create_epin_form_agent").on('submit', function (e) {
        e.preventDefault();
        var form = $(this);
        $.ajax({
            url: $('#base_url').val() + "admin/epin/create_epin_post_agent",
            type: 'POST',
            data: form.serialize(),
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function (response) {
                if (response.status == 'success') {
                    showSuccessAlert(response.message);
                    closePopup('#create_epin_modal_agent');
                    loadEpinSummaryTotal();
                    loaded_tabs = [selected_tab];
                    loadTabData(selected_tab);
                } else {
...
*/            let selected_epins = [];
        ...
        $('#allocate_epin_btn_agent').on('click', function() {
            /*if(!confirm(`${trans('are_you_sure_want_activate_epin')}`)) {
                return false;
            }
*/            let selected_epins = [];
            ...
            $.ajax({
               'method': 'POST',
               'url': $('#base_url').val()+"/admin/epin/allocate_epin_agent",
               'data': {
                   epins: selected_epins 
               }, 
               success: function(response) {
